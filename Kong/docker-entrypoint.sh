...
# Load environment configuration generated by venv-commons during build
if [ -f /home/kong/kong.env ]; then
  . /home/kong/kong.env
fi
...
if [[ "$1" == "kong" ]]; then
  ...
  all_kong_options="/usr/local/kong/bazel-bin/build/kong-dev/share/lua/5.1/kong/templates/kong_defaults.lua"
  if [ ! -f "$all_kong_options" ]; then
